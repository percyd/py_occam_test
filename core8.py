...
class Relation:
    ...
    Mirrors C++ VBMManager.cpp implementation.
    """
    variables: List[str]  # Variable abbreviations in this relation
    varlist: VariableList
    attributes: Dict[str, float] = field(default_factory=dict)
    table: Dict[State, float] = field(default_factory=dict)
    ...
    def compute_df(self, state_space: StateSpace) -> int:
        """Compute degrees of freedom"""
        df = 1  # Start with 1 as in C++
        for var in self.variables:
            df *= state_space.get_variable_cardinality(var)
        return df - 1  # Subtract 1 for constraint
        
    def compute_entropy(self, state_space: StateSpace) -> float:
